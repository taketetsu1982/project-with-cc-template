...
**責務:** Oura接続の管理、日次レコードの所有。タイムゾーン情報を保持する（日付境界の判定に使用）
**境界:** PG1では自分自身のみ。マルチユーザー対応はPG1スコープ外。PG1ではメール等のメタデータは不要
...
**責務:** Sleep Score・Readiness Score・Activity Score・HRV・安静時心拍数・睡眠ステージ比率を保持する
...
| 画面 | 種別 | 概要 | 対応PRD機能 |
|---|---|---|---|
| トレンドダッシュボード | composite | 14日デフォルト窓でフィジカル×メンタルの統合トレンドを可視化。カラーゾーンで範囲表示 | トレンドダッシュボード |
| 日次サマリー | screen | 特定日のメンタル（主観）→フィジカル（Oura）の順で表示。主観ファースト原則 | 日次サマリー |
| メンタルスコア入力 | screen | 4軸×7段階スライダー。30秒以内で完了する最小入力設計 | メンタルスコア入力 |
| Oura接続設定 | screen | OAuth2接続の管理（接続・再接続・切断） | Ouraデータ連携 |

### 主要な遷移

- トレンドダッシュボード → 日次サマリー（日付タップでドリルダウン）
- トレンドダッシュボード → メンタルスコア入力（今日の入力CTA）
- トレンドダッシュボード → Oura接続設定（設定リンク）
- 日次サマリー → メンタルスコア入力（メンタル編集）
- メンタルスコア入力 → 日次サマリー（保存完了）
...
- メンタルスコアは1〜7の整数値（4軸とも同一スケール、7段階Likert）
