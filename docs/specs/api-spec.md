...
- `correlations`: 観察的表現による相関パターン。データ不足時は空配列。処方箋ではなく観察として提示する。Pearson相関係数の絶対値が0.5以上のメトリクスペアを自動抽出し、観察的テンプレートで文章化する（例:「{metric_a}が高い日は{metric_b}が低い傾向があります」）。**データが30日未満の場合は空配列**（14日データでのPearson相関は検出力が不十分なため、擬似相関の誤認識リスクを回避する）
