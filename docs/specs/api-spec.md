...
```json
{
  ...
  "strain_recovery": {
    "description": "Strain（負荷）とRecovery（回復）の合成Z-score。各構成メトリクスのZ-scoreの平均",
    "strain": {
      "value": 0.1,
      "components": ["stress", "activity_score"]
    },
    "recovery": {
      "value": -0.2,
...
- `strain_recovery`: Strain（負荷）とRecovery（回復）の合成Z-score。各構成メトリクスのZ-scoreの平均で算出。14日分のデータ未蓄積時は `null`。Strain = stress + activity_score、Recovery = mood + energy + sleep_score + readiness_score + hrv_rmssd
- `correlations`: 観察的表現による相関パターン。データ不足時は空配列。処方箋ではなく観察として提示する。Pearson相関係数の絶対値が0.5以上のメトリクスペアを自動抽出し、観察的テンプレートで文章化する（例:「{metric_a}が高い日は{metric_b}が低い傾向があります」）。データが14日未満の場合は空配列
