...
| メソッド | パス | 概要 | 認可 | 備考 |
|---------|------|------|------|------|
| POST | `/api/v1/mental-entries` | メンタルスコアを新規作成（当日または過去日付） | SelfTracker | US-03, US-04 |
...
メンタルスコアを新規作成する。daily_recordが未作成の場合は自動作成する。
...
| フィールド | 型 | 必須 | 制約 | 説明 |
|-----------|---|:---:|------|------|
| `record_date` | date (YYYY-MM-DD) | - | 当日以前 | 対象日付。省略時は当日 |
...
**エラーケース:**
- 指定日分が既に存在: 409 `{"detail": "指定日のメンタルエントリは既に登録されています"}`
- 未来の日付を指定: 422 `{"detail": "未来の日付にはメンタルエントリを作成できません"}`
- バリデーション失敗: 422（値が1〜7の範囲外、必須フィールド欠落）

**業務ルール:**
- 当日または過去日付のみ新規作成可能。未来の日付は不可（US-04 E1: 過去のメンタルエントリが存在しない日で新規作成するケースに対応）
- 4軸すべて必須
...
**daily_recordが存在しない日:**
- 200を返し、`mental_entry: null`, `physical_score: null` とする（US-08エッジケース: 空の日次サマリーを表示するため）
- `id`は`null`、`record_date`はリクエストされた日付を返す
...
```json
{
  ...
  "strain_recovery": {
    "description": "Strain（負荷）とRecovery（回復）の合成Z-score",
    "strain": {
      "components": ["stress"]
    },
    "recovery": {
      "components": ["mood", "energy", "sleep_score", "readiness_score", "hrv_rmssd"]
    }
  },
  "correlations": [
    {
...
- `records[].strain_z`: 当日のStrain合成Z-score。Strain = stressのZ-score単軸。14日未満時は `null`
- `records[].recovery_z`: 当日のRecovery合成Z-score。Recovery = mood + energy + sleep_score + readiness_score + hrv_rmssd のZ-score平均。14日未満時は `null`
- `strain_recovery`: Strain/Recoveryの構成メトリクス定義（メタ情報）。日次の値は `records[].strain_z` / `records[].recovery_z` で返却する
- `correlations`: 観察的表現による相関パターン。データ不足時は空配列。処方箋ではなく観察として提示する。Pearson相関係数の絶対値が0.5以上のメトリクスペアを自動抽出し、観察的テンプレートで文章化する（例:「{metric_a}が高い日は{metric_b}が低い傾向があります」）。**データが30日未満の場合は空配列**（14日データでのPearson相関は検出力が不十分なため、擬似相関の誤認識リスクを回避する）。**注意**: 健康データは日次自己相関（lag-1 rho ≈ 0.2-0.3）を持つため、独立観測を仮定したPearson相関は偽陽性率が上昇する。PG1では簡易実装として標準Pearsonを使用するが、相関パターンはあくまで参考情報として提示し、因果関係を示唆しない表現を厳守する

**エラーケース:**
- window値が不正: 422 `{"detail": "windowは7, 14, 30のいずれかを指定してください"}`

**Z-score正規化の計算ルール:**
- `z = (当日の値 − 個人の直近14日移動平均) / 個人の直近14日標準偏差`
- 「直近14日」= 当日から遡った14日間の実績値。表示window（7日/14日/30日）の変更に関わらず計算基準は不変
