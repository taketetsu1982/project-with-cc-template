...
**三角形ループで回す。** reqs（企画）⇔ specs（設計）⇔ 実装は三角形で行き来します。どの頂点からでも始められます。プロトタイプから学んで reqs を更新する、specs を書きながら reqs の矛盾に気づく、いずれも正当なフローです。矛盾がある場合は最新の意思決定がある方が正となり、`<!-- human-decision -->` マーカーで人間の判断を保護します。
...
**鍵となるルール:**
- 矛盾がある場合は最新の意思決定がある方が正。`<!-- human-decision -->` マーカー優先
...
- reqs ⇔ specs ⇔ 実装は三角形で行き来する。どの頂点からでも始められる
- 不整合は Claude Code が検知して報告する。人間が同期マップを暗記する必要はない
...
三角形ループのいずれかの頂点から更新を開始する。Specとの齟齬に気づいたらSpecを先に修正し、reqsレベルの変更が必要ならユーザーに報告する。プロトタイプ・スパイクの場合は実装から始めてよいが、学びをreqs/specsに反映してからストーリーを完了とする。
...
│       ├── (infra-spec.md)      #   検証環境のインフラ構成定義（GTMフェーズで生成）
│       ├── test-spec.md         #   テスト計画
