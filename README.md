...
> **Note:** Step 4 以降は Claude Code が生成し、人が確認・調整するスタイルに変わる。HTMLエディタはブラウザで開いて視覚的に操作でき、変更は同じ `product-model.json` に保存される。

### HTMLエディタの使い方

`model-editor.html`（CMエディタ）と `screen-editor.html`（Screensエディタ）は、どちらもブラウザで開いて `product-model.json` を視覚的に編集するツール。2つのエディタが1つのJSONを共有し、それぞれ担当外のフィールドはそのまま保持する。

#### JSONファイルの接続

エディタを開いただけではデータは空。`product-model.json` を接続して使う。

1. エディタをブラウザで開く（`open docs/reqs/model-editor.html`）
2. **Connect** ボタンをクリックし、ファイル選択ダイアログで `product-model.json` を選ぶ（または JSONファイルをエディタ画面にドラッグ&ドロップ）
3. 接続されるとファイル名が表示され、**Auto** トグルが現れる

#### 保存の仕組み

- **Auto ON（デフォルト）:** 編集するたびに約0.5秒後に自動保存される。ファイル名横のドットが緑なら保存済み、オレンジなら保存中
- **Auto OFF:** 手動で `Cmd+S`（Mac）/ `Ctrl+S`（Windows）で保存する
- Auto トグルをクリックするとON/OFFを切り替えられる

#### CMエディタ（model-editor.html）

概念モデルの **entities（エンティティ）** と **actors（アクター）** を編集する。上部のタブで Entity / Actor ビューを切り替える。

**Entity ビュー:**
- キャンバス上のボックスがエンティティ。ドラッグで配置を移動できる
- ボックスをクリックすると右側にプロパティパネルが開き、名前・リレーションを編集できる
- キャンバスの空白部分をダブルクリック、またはツールバーの **+ Add** でエンティティを追加する
- エンティティ間のリレーションはプロパティパネルの Relations セクションで追加・編集する

**Actor ビュー:**
- 上部のタブでアクターを切り替える
- **Edit** ボタンでアクターの追加・削除・並べ替えができる
- キャンバス上のエンティティをクリックすると、そのアクターの CRUD 権限（Create / Read / Update / Delete × own / all）を設定できる

#### Screensエディタ（screen-editor.html）

**screens（画面）** と **transitions（画面遷移）** を編集する。上部のタブで Map / Screen ビューを切り替える。

**Map ビュー:**
- アクターごとに画面の全体マップが表示される。上部のタブでアクターを切り替える
- **+ Screen** で通常画面、**+ Composite** で複合画面（他の画面を埋め込むコンテナ）を追加する
- 画面ボックスをドラッグして配置を調整できる
- **Open Screen** ボタンで Screen ビューに遷移し、画面の詳細を編集する

**Screen ビュー:**
- 選択した画面の詳細を編集する。画面名・プロンプト（設計意図の説明）・表示するオブジェクト（エンティティの CRUD）・遷移先を定義する
- 遷移（transitions）はこのビューで追加・編集する


### Step 6：ユーザーストーリーと Specs を並列で進める

画面定義が完了したら、2つのトラックを並列で進める。
